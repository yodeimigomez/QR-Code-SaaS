...
class App {
    ...
    public function __construct() {
        ...
        if(\Altum\Authentication::check()) {
            ...
            if(isset($_GET['set_language']) && array_key_exists($_GET['set_language'], Language::$active_languages)) {
                if(Language::$name != $user->language) {
                ...
                /* Redirect */
                $request_query = http_build_query(array_diff_key($_GET, array_flip(['altum', 'set_language'])));
                redirect(\Altum\Router::$original_request . ($request_query ? '?' . $request_query : null), false, 301);
            }
